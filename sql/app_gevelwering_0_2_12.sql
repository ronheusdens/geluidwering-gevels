-- app-gevelwering DDL version 0.2.12 — 2026-07-22
-- DGMR Geluidwering Gevels (GL.cat) material catalog + R spectra (125–4000 Hz)
-- Requires 0.2.11 applied first. Seed: app_gevelwering_0_2_12_gl_material_seed.sql

CREATE TABLE IF NOT EXISTS app_gevelwering.material (
...
CREATE INDEX IF NOT EXISTS material_name_idx
  ON app_gevelwering.material (name);

CREATE INDEX IF NOT EXISTS material_category_idx
  ON app_gevelwering.material (category)
  WHERE category IS NOT NULL;

CREATE INDEX IF NOT EXISTS material_spectrum_ok_idx
  ON app_gevelwering.material (spectrum_ok);
...
COMMENT ON COLUMN app_gevelwering.material.catalog_index IS
  '0-based record order in GL.cat.';
COMMENT ON COLUMN app_gevelwering.material.material_no IS
...
COMMENT ON COLUMN app_gevelwering.material.source IS
