-- acoustics DDL version 0.2.14 — 2026-07-22
-- Rebuild acoustics.material from catalogusGG.pdf (master_category + missing columns)
-- Requires 0.2.13 applied first. Seed: acoustics_0_2_14_catalogus_gg_seed.sql

DROP TABLE IF EXISTS acoustics.material CASCADE;

CREATE TABLE acoustics.material (
...
CREATE INDEX material_name_idx ON acoustics.material (name);
CREATE INDEX material_master_category_idx ON acoustics.material (master_category);
CREATE INDEX material_category_idx ON acoustics.material (category) WHERE category IS NOT NULL;
CREATE INDEX material_spectrum_ok_idx ON acoustics.material (spectrum_ok);
...
COMMENT ON COLUMN acoustics.material.master_category IS
  'Catalog section from page header (e.g. Elementen, Glas, Ventilatievoorzieningen).';
COMMENT ON COLUMN acoustics.material.catalog_id IS
  'Catalog id such as D00118.';
COMMENT ON COLUMN acoustics.material.ra_dba IS
...
COMMENT ON COLUMN acoustics.material.source_ref IS
