 ...
 *
 * Spectrum is metadata only until a spectral R' kernel exists.
 */

export const CR_DB = 3;

/** Default grenswaarde karakteristiek binnenniveau Lbi;k (dB). */
export const GRENZWAARDE_LBIK_DB = 33;

/** @type {Record<string, number>} */
export const GRENZWAARDE_LBIK_BY_FUNCTIE = {
  Woonfunctie: 33,
...
/** @param {string|null|undefined} gebruiksfunctie */
export function grenswaardeLbik(gebruiksfunctie) {
  const key = String(gebruiksfunctie || "").trim();
  if (key && Object.prototype.hasOwnProperty.call(GRENZWAARDE_LBIK_BY_FUNCTIE, key)) {
...
/**
 * @param {{ ra_dba: number, quantity: number, kind?: 'area'|'length', label?: string }} el
 * @param {number} sRef
 */
export function partialRas(el, sRef) {
...
/**
 * @param {number[]} rasValues
 */
export function combineRprime(rasValues) {
 ...
 * @param {number} sM2
 */
export function roomCorrectionDb(volumeM3, t0s, sM2) {
 ...
 * @param {number} stotM2
 */
export function gakCorrectionDb(volumeM3, t0s, stotM2) {
  const V = Number(volumeM3);
  const T = Number(t0s);
...
/**
 * Bereken GA / Lbi / GA;k voor één verblijfsruimte.
 * @param {GaVrInput} input
 */
export function computeVrGa(input) {
  ...
  /** @type {{ label: string, kind: string, quantity: number, ra_dba: number, ras: number|null, meenemen_gak: boolean, cl_db: number, cg_db: number, area_for_s: number }[]} */
  const elements = [];
  ...
  const stot = elements
    .filter((e) => e.meenemen_gak)
    .reduce((a, e) => a + e.area_for_s, 0);
  ...
  // CL/Cg: van het grootste oppervlaktevlak (één gevelvlak-model).
  let cl = 0;
  let cg = 0;
  let bestArea = -1;
