...
def write_pg_seed(catalog: dict, path: Path, source: str = "GL.cat") -> None:
    """Idempotent upsert seed for app_gevelwering.material (DDL 0.2.12)."""
    lines: list[str] = [
        "-- Auto-generated by gl_cat_to_table.py — do not edit by hand.",
        "-- acoustics DDL 0.2.12 seed: DGMR GL.cat → app_gevelwering.material",
...
def main() -> None:
    ...
    ap.add_argument(
        ...
        type=Path,
        default=None,
        help="Output PostgreSQL seed SQL (app_gevelwering.material upserts)",
    )
    args = ap.parse_args()
