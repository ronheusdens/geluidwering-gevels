...
**PostgreSQL DDL:** [acoustics-postgres-schema.md](acoustics-postgres-schema.md) (**versioned** — through **0.2.11**)  
...
**Persistence:** PostgreSQL — see **[acoustics-postgres-schema.md](acoustics-postgres-schema.md)** (DDL **0.1.0**: `customer`, `address`, `building`). Bump that doc’s DDL version on every schema change.
...
1. Apply [acoustics-postgres-schema.md](acoustics-postgres-schema.md) **DDL 0.1.0** and wire `SetCustomer` / `SetProjectSite` / `OpenBuilding` (or equivalent) to Postgres via bppServer.
