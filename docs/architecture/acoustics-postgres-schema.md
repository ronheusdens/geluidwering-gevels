...
**Related:** [acoustics-suite-overview.md](acoustics-suite-overview.md), [server-runtime-rfc.md](../../../bppServer/docs/Architectural_aspects/09-bppserver/server-runtime-rfc.md) §2.5  
**Store:** PostgreSQL  
...
3. When implementing migrations in this app repo, name files to match (e.g. `sql/acoustics_0_2_0.sql`).
...
| File | Version |
|------|---------|
| `sql/acoustics_0_1_0.sql` | Base: customer, address, building |
| `sql/acoustics_0_2_0.sql` | Auth tables + `owner_user_id` + demo user |
| `sql/acoustics_0_2_1.sql` | Seed `ronheusdens`; claim NULL-owner rows |
| `sql/acoustics_0_2_2.sql` | Registration email + `must_change_password` + `access_request` |
| `sql/acoustics_0_2_3.sql` | `project_status` on building + seed `admin` |
| `sql/acoustics_0_2_4.sql` | Add `PROJECT_FINISHED` to `project_status` enum |
| `sql/acoustics_0_2_5.sql` | One customer per owner; project delete guards + dwelling cleanup |
| `sql/acoustics_0_2_6.sql` | `document` table — pdf/dwg blobs per project |
| `sql/acoustics_0_2_12.sql` | `material` catalog (GL.cat); seed `acoustics_0_2_12_gl_material_seed.sql` |
...
CREATE TABLE acoustics.service_user (
...
CREATE TABLE acoustics.login_session (
  id              uuid PRIMARY KEY DEFAULT gen_random_uuid(),
...
ALTER TABLE acoustics.customer
  ADD COLUMN owner_user_id uuid REFERENCES acoustics.service_user (id) ON DELETE SET NULL;

ALTER TABLE acoustics.building
  ADD COLUMN owner_user_id uuid REFERENCES acoustics.service_user (id) ON DELETE SET NULL;
```

(Full idempotent script: `sql/acoustics_0_2_0.sql`.)
...
Browser uploads **raw file bytes** to the UI server (not WebSocket). The server validates the bppServer session token against PostgreSQL and inserts one row into `acoustics.document` with a parameterized `bytea` bind.
...
| `413` | Exceeds `ACOUSTICS_MAX_UPLOAD_BYTES` (default 100 MiB) |
...
|--------|---------|--------|
| Section object | `acoustics.drawing_region` (+ view `acoustics.drawing_section`) | One row per accepted/saved section |
...
| Object | Storage | Notes |
|--------|---------|--------|
| Room / subsection | `acoustics.drawing_subsection` | Closed polyline in **section-local** 0–1 coords (`points` jsonb) |
...
| Object | Storage | Notes |
|--------|---------|--------|
| Material | `acoustics.material` | One row per GLMATR01 record |
...
DDL: `sql/acoustics_0_2_12.sql`. Seed (idempotent upsert): `sql/acoustics_0_2_12_gl_material_seed.sql` (regenerate via `docs/gl_cat_to_table.py GL.cat --sql …`). Applied by `./start.sh`.
