#!/usr/bin/env python3
"""Parse DGMR catalogusGG.pdf → tabular CSV/JSON + PostgreSQL seed for acoustics.material.
...
def write_pg_seed(rows: list[dict], path: Path) -> None:
    lines = [
        ...
        "-- acoustics DDL 0.2.14 seed: catalogusGG.pdf → acoustics.material",
        "",
        "BEGIN;",
        "DELETE FROM acoustics.material;",
        "",
    ]
    batch: list[str] = []
    ...
    def flush() -> None:
        ...
        if not batch:
            return
        col_list = ", ".join(COLS)
        lines.append(f"INSERT INTO acoustics.material ({col_list}) VALUES")
        lines.append("  " + ",\n  ".join(batch) + ";")
        lines.append("")
        batch = []
